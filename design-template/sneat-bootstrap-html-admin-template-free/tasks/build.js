...
module.exports = (conf, srcGlob) => {
    ...
    setTimeout(function () {
      webpack(require('../webpack.config'), (err, stats) => {
        if (err) {
          ...
          if (err.details)
            log(colors.gray('Webpack error details:'), err.details);
          return cb();
        }
        ...
        if (stats.hasErrors()) {
          info.errors.forEach((e) =>
            log(colors.gray('Webpack compilation error:'), colors.red(e))
          );
        }

        if (stats.hasWarnings()) {
          info.warnings.forEach((w) =>
            log(colors.gray('Webpack compilation warning:'), colors.yellow(w))
          );
        }
  ...
  const FONT_TASKS = [
    ...
    const functionName = `buildFonts${font.name.replace(/^./, (m) =>
      m.toUpperCase()
    )}Task`;
